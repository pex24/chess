...
public class Bishop implements Piece{
    ...
    double x;
    double y;
    private ArrayList<Square> checkingAttacks = new ArrayList<Square>();
    
    public Bishop(Square startingPosition, Player player, gameSetup setup) {
        this.setup = setup;
        this.player = player;
    ...
    public ArrayList<Square> possibleMoves() {
        ...
        Square curr = this.currentSquare;
        ArrayList<ArrayList> diagonalArrays = gameBoard.getDiagonals(curr);
        for (ArrayList<Square> arr : diagonalArrays) {
            for (Square square : arr) {
                if (gameBoard.isLegalMove(square.getRow(), square.getCol())) {
    ...
    public void drawPiece(Graphics g) throws IOException {
        if (this.color == Color.BLACK) {
            Image img = ImageIO.read(new FileInputStream("pieceImages/blackBishop.png"));
            g.drawImage(img, (int)this.x, (int)this.y, null);    
        }
        if (this.color == Color.WHITE) {
            Image img = ImageIO.read(new FileInputStream("pieceImages/whiteBishop.png"));
            g.drawImage(img, (int)this.x, (int)this.y, null); 
        }
