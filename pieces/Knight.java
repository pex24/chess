...
public class Knight implements Piece {
    ...
    public Knight(Square startingPosition, Player player, gameSetup setup) {
        this.setup = setup;
        this.player = player;
    ...
    public ArrayList<Square> possibleMoves() {
        ...
        int[][] possibles = {{-2,-1},{-2,1},{2,-1},{2,1},{-1,-2},{1,-2},{-1,2},{1,2}};
        for (int[] i : possibles) {          
            int row = i[0];
            int col = i[1];
                       
            if (gameBoard.isLegalMove(currentSquare.getRow() + row, currentSquare.getCol() + col)) {
                Square square = gameBoard.getTile(currentSquare.getRow() + row, currentSquare.getCol() + col);
                if (!square.containsPiece()) {
                    possibleMoves.add(square);
                }
                else if (square.selectPiece().belongsToPlayer() != this.player) {
                    possibleMoves.add(square);
    ...
    public void drawPiece(Graphics g) throws IOException {
        if (this.color == Color.BLACK) {
            Image img = ImageIO.read(new FileInputStream("pieceImages/blackKnight.png"));
            g.drawImage(img, (int)this.x, (int)this.y, null);    
        }
        if (this.color == Color.WHITE) {
            Image img = ImageIO.read(new FileInputStream("pieceImages/whiteKnight.png"));
            g.drawImage(img, (int)this.x, (int)this.y, null); 
        }
    }
